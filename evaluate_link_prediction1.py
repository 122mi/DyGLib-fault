...
if __name__ == "__main__":
    ...
    # df = pd.DataFrame(columns=['run','validate loss','validate average_precision','validate roc_auc','new node validate loss','new node average_precision','new node roc_auc','test loss','node average_precision','test average_precision','node roc_auc','test roc_auc','new node test loss','new node test average_precision','new node test roc_auc'])#列名
    # df.to_csv('./evaluate_results/guzhang_dyg.csv',index=False) 

    df = pd.DataFrame(columns=['run','test loss','node average_precision','test average_precision','node roc_auc','test roc_auc','new node test loss','new node test average_precision','new node test roc_auc'])#列名
    df.to_csv('./evaluate_results/uci_TGN.csv',index=False) 
    ...
    if args.model_name == 'EdgeBank':
        ...
        for run in range(args.num_runs):
            ...
            # logger.info(f'src_node_id: {test_data.src_node_ids}')
            # logger.info(f'edge_id: {test_data.edge_ids}')
            ...
            for metric_name in test_metrics[0].keys():
                ...
                logger.info(f'node {metric_name}, {[test_metric[metric_name] for test_metric in test_metrics]}')
            ...
            # validate_loss=round(np.mean(val_losses),4)
            # for metric_name in val_metrics[0].keys():
            #     val_average_precision=round(np.mean([val_metric['average_precision'] for val_metric in val_metrics]),4)
            #     val_roc_auc=round(np.mean([val_metric['roc_auc'] for val_metric in val_metrics]),4)
            # new_node_validate_loss=round(np.mean(new_node_val_losses),4)
            # for metric_name in new_node_val_metrics[0].keys():
            #     new_node_val_average_precision=round(np.mean([new_node_val_metric['average_precision'] for new_node_val_metric in new_node_val_metrics]),4)
            #     new_node_val_roc_auc=round(np.mean([new_node_val_metric['roc_auc'] for new_node_val_metric in new_node_val_metrics]),4)
            # test_loss=round(np.mean(test_losses),4)
            # for metric_name in test_metrics[0].keys():
            #     node_test_average_precision=[test_metric['average_precision'] for test_metric in test_metrics]
            #     test_average_precision=round(np.mean([test_metric['average_precision'] for test_metric in test_metrics]),4)
            #     node_test_roc_auc=[test_metric['roc_auc'] for test_metric in test_metrics]
            #     test_roc_auc=round(np.mean([test_metric['roc_auc'] for test_metric in test_metrics]),4)
            # new_node_test_loss=round(np.mean(new_node_test_losses),4)
            # for metric_name in new_node_test_metrics[0].keys():
            #     new_node_test_average_precision=round(np.mean([new_node_test_metric['average_precision'] for new_node_test_metric in new_node_test_metrics]))
            #     new_node_test_roc_auc=round(np.mean([new_node_test_metric['roc_auc'] for new_node_test_metric in new_node_test_metrics]))

            # list = [run,validate_loss,val_average_precision,val_roc_auc,new_node_validate_loss,new_node_val_average_precision,new_node_val_roc_auc,test_loss,node_test_average_precision,test_average_precision,node_test_roc_auc,test_roc_auc,new_node_test_loss,new_node_test_average_precision,new_node_test_roc_auc]
            # data = pd.DataFrame([list])
            # data.to_csv('./evaluate_results/guzhang_dyg.csv', mode='a', header=False, index=False)
            ...
            list = [run,test_loss,node_test_average_precision,test_average_precision,node_test_roc_auc,test_roc_auc,new_node_test_loss,new_node_test_average_precision,new_node_test_roc_auc]
            data = pd.DataFrame([list])
            data.to_csv('./evaluate_results/uci_TGN.csv', mode='a', header=False, index=False)
            ...
            if args.model_name not in ['JODIE', 'DyRep', 'TGN']:
                result_json = {
                    ...
                    "test metrics": {metric_name: f'{test_metric_dict[metric_name]:.4f}' },
