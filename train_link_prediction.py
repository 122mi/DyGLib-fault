...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        result_folder = f"./train_results/{args.model_name}/{args.dataset_name}"
        os.makedirs(result_folder, exist_ok=True)
        result_path = os.path.join(result_folder, f"{args.save_model_name}.csv")
        df = pd.DataFrame(columns=['epoch','train loss','train average_precision','train roc_auc','validate loss','validate average_precision','validate roc_auc','new node validate loss','new node average_precision','new node roc_auc'])#列名
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    raise ValueError(f"Wrong value for model_name {args.model_name}!")
                # logger.info(f'{batch_src_node_embeddings}')
                ...
                #     src_node_tensor = batch_src_node_embeddings.tolist()
                    # with open('src_node_tensor.csv', 'w', newline='') as f1:
                    #     writer = csv.writer(f1)
                    #     writer.writerows(src_node_tensor)
                    # dst_node_tensor = batch_dst_node_embeddings.tolist() 
                    # with open('dst_node_tensor.csv', 'w', newline='') as f2:
                    #     writer = csv.writer(f2)
                    #     writer.writerows(dst_node_tensor)  
                # logger.info(f'{batch_src_node_embeddings}')
                if args.model_name in ['JODIE', 'DyRep', 'TGN']:
            ...
            for metric_name in train_metrics[0].keys():
                train_average_precision=round(np.mean([train_metric['average_precision'] for train_metric in train_metrics]),4)
                train_roc_auc=round(np.mean([train_metric['roc_auc'] for train_metric in train_metrics]),4)
            ...
            list = [epoch,train_loss,train_average_precision,train_roc_auc,validate_loss,val_average_precision,val_roc_auc,new_node_validate_loss,new_node_val_average_precision,new_node_val_roc_auc]
